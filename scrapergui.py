...
s = 0
path = '.'
def requests_image(file_url,s,m,t,o,k,file_name,path):
	thepath = path + '/' + k + '/' + o + '/'
	...
	if not os.path.exists(path + file_name):
		...
		if i.status_code == requests.codes.ok:
			...
			return False
def requests_video(file_url,s,o,k,file_name,path):
...
def ilec(url,s,path):
	...
	ii = 0
	for jj in alist:
		requests_video(blist[ii],s,dir,title,jj,path)
		ii = ii+1

def scraperec(m,t,s,o,k,visitlist,path):
	url = 'https://lms.curtin.edu.au/webapps/blackboard/content/listContent.jsp?course_id=_' + m + '_1&content_id=_' + t + '_1&mode=reset'
	r = s.get(url)
	...
	soup = BeautifulSoup(data)
 	for pdf in soup.find_all('a'):
		...
		if ".pdf" in w or "xid" in w:
			...
			requests_image('https://lms.curtin.edu.au/' + w,s,m,t,o,k,name.replace(' ',''),path)
	for link in soup.find_all('a'):
		l = link.get('href')
		if l.startswith('https://lms.curtin.edu.au/webapps/blackboard/content/listContent.jsp?') or l.startswith('/webapps/blackboard/content/listContent.jsp?'):
			l = l.replace('https://lms.curtin.edu.au/webapps/blackboard/content/listContent.jsp?course_id=_' + m + '_1&content_id=_','')
			l = l.replace('/webapps/blackboard/content/listContent.jsp?course_id=_' + m + '_1&content_id=_','')
			l = l.replace('_1&mode=reset','')
			l = l.replace('_1','')
			C = l
			if C not in visitlist:
				visitlist.append(C)
				visitlist = scraperec(m,l,s,link.span.string,k,visitlist,path)
	return visitlist

def scrape(m,s,o,path):
	visitlist = []
	r = s.get('https://lms.curtin.edu.au/webapps/blackboard/execute/launcher?type=Course&id=_' + m + '_1')	
	data = r.text
	soup = BeautifulSoup(data)
  	for pdf in soup.find_all('a'):
		w = pdf.get('href')
		if ".pdf" in w or "xid" in w:
			...
			if '1slideperpage' in name or '4slideperpage' in name:
				name = urlsplit(w)[2].split('/')[-1] + '.pdf' #fuck dave
			requests_image('https://lms.curtin.edu.au/' + w,s,m,t,'',o,name.replace(' ',''),path)
	for link in soup.find_all('a'):
		l = link.get('href')
		if l.startswith('https://lms.curtin.edu.au/webapps/blackboard/content/listContent.jsp?'):
			...
			if C not in visitlist:
				visitlist.append(C)
				visitlist = scraperec(m,l,s,link.span.string,o,visitlist,path)

def login(user, password):
	unitlist = []
	ileclist = []
	password = base64.b64encode(password)
	payload = {'login': 'Login', 'action': 'login', 'user_id': user, 'encoded_pw': password}
	url = 'https://lms.curtin.edu.au/webapps/login/'

	with requests.Session() as s:
		s.post(url, data = payload)
		r = s.get('https://lms.curtin.edu.au/webapps/portal/execute/tabs/tabAction?tab_tab_group_id=_3_1')
		data = r.text
		soup = BeautifulSoup(data)
	...
	for link in soup.find_all('a'):
		...
		if l.startswith(' /webapps/portal/frameset.jsp?tab_tab_group_id=_4_1&url=%2Fwebapps%2Fblackboard%2Fexecute%2Flauncher%3Ftype%'):
			l = l.replace(' /webapps/portal/frameset.jsp?tab_tab_group_id=_4_1&url=%2Fwebapps%2Fblackboard%2Fexecute%2Flauncher%3Ftype%3DCourse%26id%3D_','')
			l = l.replace('_1%26url%3D','')
			unitlist.append([l,link.string])
	for unit in unitlist:
		r = s.get('https://lms.curtin.edu.au/webapps/blackboard/execute/launcher?type=Course&id=_' + unit[0] + '_1')
		data = r.text
		soup = BeautifulSoup(data)
		for link in soup.find_all('a'):
			if "Echo" in link.get('href'):
				ileclist.append([link.get('href'),soup.find(id='courseMenu_link').get('title')[9:]])
...
class scrapergui(Frame):
	def __init__(self,Master=None,**kw):
		...
		self.__Button3.bind('<ButtonRelease-1>',self.__on_Button3_ButRel_1)
		...
		self.__Listbox1 = Listbox(self.__Frame4, width=40,selectmode=EXTENDED)
		...
		self.__Entry2.bind('<KeyRelease-Return>',self.__on_Button2_ButRel_1)
		self.__Button2 = Button(self.__Frame1,text='Login',width=20)
		self.__Button2.pack(side='top',padx=5,pady=5)
		self.__Button2.bind('<ButtonRelease-1>',self.__on_Button2_ButRel_1)
		self.__Button1.bind('<ButtonRelease-1>',self.__on_Button1_ButRel_1)
		...
		self.__Button5 = Button(self.__Frame7,text='goto url',width=20)
		...
		self.__Button4.bind('<ButtonRelease-1>',self.__on_Button4_ButRel_1)
	...
	def __on_Button5_ButRel_1(self, Event=None):
		...
		for lecs in map(int,self.__Listbox2.curselection()):
			url = ileclist[lecs][0]
			webbrowser.open('https://lms.curtin.edu.au' + url, new=1, autoraise=True)
